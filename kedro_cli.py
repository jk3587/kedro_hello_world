...
def __get_kedro_context__():
    ...
    from kedro_hello_world.run import __kedro_context__
...
def run(tag, env, parallel, runner):
    """Run the pipeline."""
    from kedro_hello_world.run import main
...
def build_docs():
    ...
    python_call(
        "ipykernel", ["install", "--user", "--name=kedro_hello_world"]
    )
